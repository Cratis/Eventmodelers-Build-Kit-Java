...
description: See a slice go from Planned to Done with the Eventmodelers Build Kit - Java
tableOfContents: false
---

# Run Your First Slice

This walkthrough shows a slice going from `Planned` on your Eventmodelers board to a tested, working Cratis vertical slice in your Java project — with no manual coding in between.
...
- One type per slice, with the command/event/reactor together
...
Because the slice only carries `commands`/`events` (no `processors`, no `projections`/`queries`), the loop routes to the `build-state-change` skill, which generates `Authors/Registration/RegisterAuthor.java`:
...
public record RegisterAuthor(@CommandKey String id, String name) {
    public AuthorRegistered handle() {
        return new AuthorRegistered(new AuthorName(name));
    }
}
